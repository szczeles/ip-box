...
class UpworkTimeEntriesProvider:
    def __init__(self, client):
        self.client = client
        self.user_ref = upwork.routers.auth.Api(client).get_user_info()["info"]["ref"]
        self.user_id = upwork.routers.organization.users.Api(client).get_specific(
            self.user_ref
        )["user"]["id"]
    ...
    def get_invoices(self, start_date, end_date):
        ...
        results = upwork.routers.reports.finance.earnings.Gds(
            self.client
        ).get_by_freelancer(freelancer_reference=self.user_ref, params={"tq": query})
    ...
    def get_time_entries(self, invoice):
        ...
        results = upwork.routers.reports.time.Gds(self.client).get_by_freelancer_full(
            freelancer_id=self.user_id, params={"tq": query}
        )
        for row in sorted(results["table"]["rows"], key=lambda r: r["c"][0]["v"]):
            yield TimeEntry(
                date=f"{row['c'][0]['v'][:4]}-{row['c'][0]['v'][4:6]}-{row['c'][0]['v'][6:8]}",
...
class MultiProjectReportWriter:
    ...
    def write(self, entry):
        if not entry.invoice.project in self.writers:
            self.writers[entry.invoice.project] = ReportWriter(
                self.get_path(entry.invoice.project)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--access-token-secret", help="Access token secret", required=False
    )
    ...
    if args.access_token is not None and args.access_token_secret is not None:
        ...
        client = upwork.Client(
            upwork.Config(
                {"consumer_key": args.api_key, "consumer_secret": args.api_secret}
            )
        )
        verifier = input(
            "Please enter the verification code you get "
            "following this link:\n{0}\n\n> ".format(client.get_authorization_url())
        )
